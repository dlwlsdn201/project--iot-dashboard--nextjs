...
export function CarbonEmissionWidget() {
  ...
  return (
    <Card title="Carbon Emission — tCO₂" className="h-full">
      <div className="mb-3">
      ...
      <ResponsiveContainer width="100%" height={165}>
        <BarChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          ...
          <Tooltip
            ...
            itemStyle={{ fontFamily: 'var(--font-jetbrains-mono)', color: '#10b981' }}
            cursor={{ fill: '#1e293b' }}
          />
          <Bar dataKey="tCO2" name="tCO₂" isAnimationActive={false} radius={[3, 3, 0, 0]}>
            {data.map((_, index) => (
              <Cell key={index} fill="#10b981" fillOpacity={0.7 + (index / data.length) * 0.3} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
